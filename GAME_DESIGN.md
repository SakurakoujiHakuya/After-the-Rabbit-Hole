...
1. `兔子洞没有底`：30 秒随机生存，平台持续加速上升，只控制左右落点；终点为 `DRINK ME` 长桌、金钥匙与门厅光。
2. `所有门都太小`：钥匙与机关门。
...
- 非迷宫下坠关使用 `mode: "fall"` 与 `fallConfig`；生成器必须先构造仅含普通平台的可达安全链，再添加脆弱或尖刺支路。滚动进度由有效游戏时间计算，受伤复位动态选择当前可见安全平台，不设置固定检查点；终点着陆点用 `landing` 标记主题表现。
