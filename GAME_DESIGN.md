...
8. `镜子说反话`：女王花园后的镜中支线；棋子切换黑白棋路，两枚碎片只在对应状态出现，镜面不再直接反转操控。
9. `女王的槌球场`：女王花园后的槌球支线；必须由对应火烈鸟发射并真实穿过顺序球门，得分后才开启下一段赛道。
10. `名字的审判`：两条后期支线重新汇合，将前述机关与旋转法庭组合成最终考验。

## 分支与存档

第三章后可以选择蘑菇森林或茶会；女王花园后可以选择镜中走廊或槌球场。每个分支点独立保存选择，解锁章节、最佳时间、死亡次数和当前章节保存在浏览器 `localStorage` 中。完成主线后，未走过的支线会在“梦境书签”中开放。分支选择还会继承到共同终章：蘑菇路线提供缩小捷径，茶会路线提供短暂停表；镜中与槌球路线分别留下不同外观的一次性防护纪念物。
