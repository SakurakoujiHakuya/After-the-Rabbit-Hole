...
2. `所有门都太小`：钥匙与机关门。
3. `房子不肯容下她`：喝药缩小、穿过迷你门、拾取折扇，并在横向风中寻找低矮门闩。
4. `眼泪会记得方向`：交替水流、冰面与漩涡。
5. `每个人都赢了`：渡渡鸟荒诞竞赛；沿环形水流依次踩过四枚赛点，跑完一圈领取自己的顶针。
6. `你是谁？`：两秒前的倒影、四秒记忆接力与故事分支。
7. `一口太大，一口太小`：变大、变小与重量机关。
8. `时间卡在六点`：成对茶杯传送、轨道怀表与旋转茶桌。
9. `盘子比道理飞得快`：两条早期分支汇合；胡椒风、飞盘与此前获得的缩小或停表能力共同构成厨房路线。
10. `猫消失以后，微笑还在`：三段移动猫雾、阶段纸牌与两半微笑。
11. `把白玫瑰涂成红色`：先取得红漆，再在纸牌巡逻中依次染红三朵白玫瑰。
12. `门在你的倒影身后`：整关初始左右反转，收集定向镜片后往返三层镜盘。
13. `球门总在临时改口`：对应火烈鸟发射并真实穿过顺序球门。
14. `花色必须排成一句话`：两条后期支线汇合；按花色证词顺序折返迷宫。
15. `谁偷走了她的名字`：将前述机关与旋转法庭组合成最终考验。

## 分支与存档

第六章后可以选择蘑菇森林或茶会；女王花园后可以选择镜中走廊或槌球场。每条实际通关路线包含 13 章，总章节书签包含 15 张地图。每个分支点独立保存选择，解锁章节、最佳时间、死亡次数和当前章节保存在浏览器 `localStorage` 中。完成主线后，未走过的支线会在“梦境书签”中开放。分支选择会在胡椒厨房、柴郡猫森林、纸牌游行和共同终章持续生成不同道具：蘑菇路线提供缩小能力，茶会路线提供短暂停表；镜中与槌球路线分别留下不同外观的一次性防护纪念物。
